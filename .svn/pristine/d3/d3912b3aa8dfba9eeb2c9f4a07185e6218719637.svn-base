...
public class PublicSignsBoardInfoServiceImpl implements PublicSignsBoardInfoService{
	...
	@Override
	public List<PublicSignsBoardInfoBean> save(List<PublicSignsBoardInfoBean> beans) {
		List<PublicSignsBoardInfoBean> returnList = new ArrayList<PublicSignsBoardInfoBean>();
		
		for(PublicSignsBoardInfoBean bean:beans){
			
			PublicSignsBoardInfoBean returnBean = save(bean);
			if(returnBean != null){
	 ...
	 * @return 返回不重复的
	 */
	public PublicSignsBoardInfoBean save(PublicSignsBoardInfoBean bean){
		
		if(StringUtils.isBlank(bean.getBoardCode())){
			return null;
		}
		
		PublicSignsBoardInfo infoInDb = dao.findByBoardCode(bean.getBoardCode());
		
		if(infoInDb!=null){
			return null;
		}
		
		infoInDb = new PublicSignsBoardInfo(); 
		
		BeanUtils.copyProperties(bean, infoInDb);
		
		PublicSignsBoardInfo info = dao.save(infoInDb);
