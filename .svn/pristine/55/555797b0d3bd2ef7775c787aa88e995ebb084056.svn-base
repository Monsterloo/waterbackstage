...
public class LoginController {
	...
	public String adminLogin(UserBean user, HttpSession session,HttpServletRequest request ) {
		String IsScan=(String)request.getParameter("IsScan");
		Subject subject = SecurityUtils.getSubject();
		UsernamePasswordToken token = new UsernamePasswordToken(user.getUsername(),user.getPassword());
		if("1".equals(IsScan))
		{
			String UKey=request.getParameter("UKey");
			SwjUserBean ScanUser=(SwjUserBean)request.getServletContext().getAttribute("UKey-"+UKey);
			token=new UsernamePasswordToken(ScanUser.getLoginname(),ScanUser.getPassword());
		}
	...
	@RequestMapping(value = "/ScanLogin", method = RequestMethod.POST, produces={"application/json;charset=UTF-8"})
