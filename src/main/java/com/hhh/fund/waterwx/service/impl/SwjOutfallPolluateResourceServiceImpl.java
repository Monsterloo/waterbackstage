...
public class SwjOutfallPolluateResourceServiceImpl implements SwjOutfallPolluateResourceService{
	...
	@Override
	public List<OutfallPolluateResourceBean> saveList(List beanList) {
		List<OutfallPolluateResourceBean>  beans = new ArrayList<OutfallPolluateResourceBean>();
		
		for(Object obj:beanList){
		
			OutfallPolluateResourceBean bean = (OutfallPolluateResourceBean)obj;
			
			OutfallPolluateResource onlyOneRecord = swjOutfallPolluateResourceDao.findOnlyOneRecord(bean.getRivercode(),bean.getArea(),bean.getOutfalltype(),bean.getPolldescription());
			if(onlyOneRecord != null){
				continue;
			}
			...
			if(info.getCreateTime()==null&&StringUtils.isBlank(info.getDrainageTo())&&StringUtils.isBlank(info.getPolldescription())){
				continue;
			}
			
			bean.setId(swjOutfallPolluateResourceDao.save(info).getId());
			beans.add(bean);
		}
