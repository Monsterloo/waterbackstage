...
public class ResponsibilityServiceImpl implements ResponsibilityService {
	...
	@Override
	public List<ResponsibilityBean> saveList(List beanList) {
		
		List<ResponsibilityBean> beans = new ArrayList<ResponsibilityBean>();
		for(Object obj:beanList){
			ResponsibilityBean bean = (ResponsibilityBean)obj;
			
			Responsibility responsibility = responseDao.findOnlyOneRecord(bean.getRiverCode(),bean.getPartName(),bean.getLeftRight());
			...
			if(info.getAreaName()==null&&info.getDistMgrName()==null&&info.getManageMgrName()==null){
				continue;
			}
			
			bean.setId(responseDao.save(info).getId());
			beans.add(bean);
		}
		return beans;
	}
